...
from src.scripts.surgical_robotics_challenge.utils.utilities import *
...
import src.scripts.surgical_robotics_challenge.units_conversion
...
class Task_1_Evaluation_Report():
    ...
    def print_report(self):
        ...
        if self.success:
            ...
            print('\t Task 1 Overall Score (Lower is Better): ',
                  self.E_base + self.E_mid + self.E_tip)
        else:
...
class Task_2_Evaluation_Report():
    ...
    def print_report(self):
        ...
        if self.success:
            print(OK_STR('\t Task Successful: '))
            print('\t Completion Time: ', self.completion_time)
            print('\t Targeted Entry/Exit Hole Pair (1 to 4): ',
                  self.entry_exit_idx + 1)
            print('\t Needle Tip Axial Distance From Exit Hole (Recommended 0.05): ',
...
class SceneKinematicsFrame:
    def __init__(self):
        ...
        self.T_holesINw[HoleType.ENTRY] = [Frame()
                                           for _ in range(GlobalParams.hole_count)]
        self.T_holesINw[HoleType.EXIT] = [Frame()
                                          for _ in range(GlobalParams.hole_count)]
        self.T_ntINw = Frame()
        self.t = 0.0
...
class ContactEventHelper:
    ...
    def validate_needle_insertion_events(needle_holes_proximity_events):
        ...
        for hole_type in HoleType:
            for hidx in range(GlobalParams.hole_count):
                event_count = len(
                    needle_holes_proximity_events[hole_type][hidx])
        ...
        print('Total Events: ', total_events,
              ' Incorrect Events: ', incorrect_events)
            ...
            for hidx in range(GlobalParams.hole_count):
                ...
                if event_size < 2:
                    ...
                    for ev in range(event_size-1, 0, -1):
                        ...
                        z0 = needle_holes_proximity_events[hole_type][hidx][ev -
                                                                            1].T_ntINhole.p[2]
                    ...
                    if i_insertion == -1:
                        ...
                        for ev in range(event_size):
                            ...
                            depths.append(z)
                        print('Error! For hole_type',
                              hole_type, 'hole_idx', hidx)
...
class Task_2_Evaluation():
    def __init__(self, client, team_name):
        ...
        for i in range(GlobalParams.hole_count):
            self._hole_objs[HoleType.ENTRY].append(
                client.get_obj_handle("Entry" + str(i+1)))
            self._hole_objs[HoleType.EXIT].append(
                client.get_obj_handle("Exit" + str(i+1)))
    ...
    def compute_needle_hole_proximity_event(self, SKF):
        ...
        for hole_type in HoleType:
            for hidx in range(GlobalParams.hole_count):
                T_ntINhole = SKF.T_holesINw[hole_type][hidx].Inverse(
                ) * SKF.T_ntINw
                ...
                if ne.is_needle_intersecting_with_hole(T_ntINhole):
                    ...
                    (self._needle_holes_proximity_events[hole_type][hidx]).append(
                        ne)
    ...
    def evaluate(self):
        ...
        if NCE.hole_type is HoleType.EXIT:
            ...
            if len(insertion_events) < 2:
                # Failed
                print('Failed Task, Number of hole insertion events = ',
                      len(insertion_events), '/2')
            else:
                ...
                if event0.hole_type is NCE.hole_type and event0.hole_idx == NCE.hole_idx:
                    if event1.hole_type is HoleType.ENTRY and event1.hole_idx == NCE.hole_idx:
                        ...
                        self._report.L_ntINentry_lateral = ContactEventHelper.compute_lateral_distance_from_hole(
                            event1.T_ntINhole)
                        self._report.P_max_ntINexit_lateral = ContactEventHelper.compute_max_lateral_component_from_hole(
                            event0.T_ntINhole)
                        self._report.P_max_ntINentry_lateral = ContactEventHelper.compute_max_lateral_component_from_hole(
                            event1.T_ntINhole)
                        ...
                        print('Closest Type: ', NCE.hole_type,
                              ' Idx: ', NCE.hole_idx)
                        print('Event1 Type: ', event1.hole_type,
                              ' Idx: ', event1.hole_idx)
                else:
                    # Failed
                    print(
                        'Failed Task, Exit hole type / idx mismatch from closest type / idx')
                    print('Closest Type: ', NCE.hole_type,
                          ' Idx: ', NCE.hole_idx)
                    print('Event0 Type: ', event0.hole_type,
                          ' Idx: ', event0.hole_idx)
        else:
...
class Task_3_Evaluation_Report():
    def __init__(self):
        ...
        # Cross-sectional distance from the exit hole's center
        self.L_ntINexit_lateral = [
            None for _ in range(GlobalParams.hole_count)]
    ...
    def print_report(self):
        ...
        if self.success:
            print(OK_STR('\t Task Successful: '))
            print('\t Completion Time: ', self.completion_time)
            print('\t Needle Tip Axial Distance From Exit Hole (4/4) (Recommended 0.05): ',
                  self.L_ntINexit_axial)
            for hidx in range(GlobalParams.hole_count):
                print('--------------------------------------------')
                print('\t Hole Number: ', hidx + 1,
                      '/', GlobalParams.hole_count)
                print('\t Needle Tip Lateral Distance From Exit Hole During Insertion (Lower is Better): ',
    ...
    def evaluate(self):
        ...
        if NCE.hole_type is HoleType.EXIT:
            ...
            if len(insertion_events) < 8:
                # Failed
                print('Failed Task, Number of hole insertion events =',
                      len(insertion_events), 'out of 8')
                for ie in insertion_events:
                    print('\t Successful insertion into',
                          ie.hole_type, ie.hole_idx)
            else:
                self._report.L_ntINexit_axial = ContactEventHelper.compute_axial_distance_from_hole(
                    NCE.T_ntINhole)
                self._report.success = True
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', action='store', dest='team_name',
                        help='Team Name', default='test_team')
    parser.add_argument('-e', action='store', dest='task_evaluation',
                        help='Task to evaluate (1,2 or 3)')
